...
def prepare(t5: HFEmbedder, clip: HFEmbedder, img: Tensor, prompt: str | list[str]) -> dict[str, Tensor]:
    ...
    return {
        "img": img,
        "img_ids": img_ids.to(img.device), # serves as a placeholder for the image ids
        "txt": txt.to(img.device),
        "txt_ids": txt_ids.to(img.device), # serves as a placeholder for the prompt
...
def denoise(
    ...
    **kwargs
    ...
    print(img.shape, img_ids.shape, txt.shape, txt_ids.shape, vec.shape)
    ...
    for i, (t_curr, t_prev) in enumerate(zip(timesteps[:-1], timesteps[1:])):
        ...
        img = img + (t_prev - t_curr) * pred # [1, 4096, 64]
