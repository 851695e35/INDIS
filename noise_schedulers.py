...
class NoiseScheduleVP:
    ...
    def marginal_std(self, t): # sigma_t
        ...
        std_dev = torch.sqrt(1. - torch.exp(2. * self.marginal_log_mean_coeff(t)))
        # Ensure the output shape matches the input t shape
        return std_dev.view_as(t)
...
# dummy class for OT
class NoiseScheduleOT:
    def __init__(self, schedule='time_uniform'):
        self.schedule = schedule
        # dummy terms , disproportional to time.
        self.lambda_min = 1.0
        self.lambda_max = 0

    def inverse_lambda(self, lamb):
        return torch.tensor(lamb)
