...
class DPM_Solver(ODESolver):
    ...
    def dpm_solver_first_update(self, x, s1, s2, t1, c1, model_s=None):
        ...
        x_t = (
            ...
            - (sigma_t * phi_1) * model_s * c1 # amedcn
        )
        # print(x_t)
        # input("1")
        return x_t 
    ...
    def singlestep_dpm_solver_update(self, x, s1, s2, t1, order, r1=0.5, r2=0.5, c1=1, model_s=None):
        if order == 1:
            # print(s1, s2, t1, c1)
            # print(s1.requires_grad, s2.requires_grad, t1.requires_grad, c1.requires_grad)
            # input("1")
            x_t = self.dpm_solver_first_update(x, s1, s2, t1, c1, model_s=model_s)
        elif order == 2:
            x_t = self.dpm_solver_second_update(x, s1, s2, t1, r1, r2, c1, model_s=model_s)
        else:
    ...
    def sample_simple(self, model_fn, x, timesteps, timesteps2, cn, order=2, NFEs=None, rs=None, rs2=None,  condition=None, unconditional_condition=None, **kwargs):
        '''
        ...
        for step, od in enumerate(orders):
            ...
            x = self.singlestep_dpm_solver_update(x, s1, s2, t1, od, r1, r2, c1)
        return x
