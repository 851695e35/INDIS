...
try:
    from flash_attn import flash_attn_func # Use the functional interface from v2
    FLASH_ATTN_AVAILABLE = True
except ImportError:
    print("FlashAttention v2 is not installed. Falling back to legacy attention.")
    # Define a dummy function or handle the fallback elsewhere if needed
...
class QKVFlashAttention(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        ), "self.embed_dim must be divisible by num_heads"
        self.head_dim = self.embed_dim // num_heads
        assert self.head_dim % 8 == 0 and self.head_dim <= 128, "FlashAttention only supports head_dim <= 128 and divisible by 8"

        self.rearrange = rearrange

    def forward(self, qkv, attn_mask=None, key_padding_mask=None, need_weights=False):
        if not FLASH_ATTN_AVAILABLE:
            raise ImportError("FlashAttention v2 is required for QKVFlashAttention but not installed.")
        ...
        attn_output = flash_attn_func(
            q, k, v, dropout_p=self.attention_dropout, causal=self.causal
        )

        return self.rearrange(attn_output, "b s h d -> b (h d) s")
